...
LastEditors: Kumo
LastEditTime: 2024-09-28 17:16:15
...
class MSAuth:
    def __init__(
        ...
        redirect_uri,
        scopes,
        refresh_token_path="_refresh_token",
    ):
        ...
        self.client = microsoftgraph.client.Client(
            client_id, client_secret=client_secret, account_type="common"
        )

    def get_access_token(self):
        if os.path.exists(self._refresh_token_path):
            refresh_token = open(self._refresh_token_path, "r").read()
            try:
                self._token = self.client.refresh_token(
                    self._redirect_uri, refresh_token
                ).data
        ...
        if "refresh_token" in self._token:
            open(self._refresh_token_path, "w").write(self._token["refresh_token"])
    ...
    def _call_user_web_login(self):
        url = self.client.authorization_url(
            self._redirect_uri, self._scopes, state=None
        )
